...
## Python 扩展
  ...
  `cjdb` 继承了 `lldb` 使用 `Python` 脚本来增强调试能力以及自定义调试功能的能力。例如开发者可以自定义命令或者扩展自定义格式化输出。详情请参见 `lldb` 官方文档。
...
5. 如果开发者在高于 `llvm15.0.4` 版本的系统环境上运行，可能会遇到不兼容的问题和风险，如 `C` 语言互操作场景，`cjdb` 无法正常解析 `C` 代码的文件和行号信息。
