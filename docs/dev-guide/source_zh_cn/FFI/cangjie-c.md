...
函数调用约定描述调用者和被调用者双方如何进行函数调用（如参数如何传递、栈由谁清理等），函数调用和被调用双方必须使用相同的调用约定才能正常运行。仓颉编程语言通过 `@CallingConv` 来表示调用约定，目前只支持 `CDECL`，表示 clang 的 C 编译器在不同平台上默认使用的调用约定，未来可能根据场景需要扩充其他调用约定的支持。
...
关于仓颉编译器支持的所有编译选项，详情请参见[cjc 编译选项](../Appendix/compile_options.md)。
